...
from .utils import cryptosecure_string_generator
from .validators import PHONE_REGEX


class UtilsTest(TestCase):
    """ Suite to test the cryptosecure string generator is working properly.

    """

    def test_crypto_default_length(self):
        """ Test if the crypto_secure string generator, creates a 12 char
        string, when given no arguments.

        """

        test_result = cryptosecure_string_generator()
        self.assertEqual(len(test_result), 12)

    def test_crypto_custom_length(self):
        ...
        for x in range(1, 20):
            test_result = cryptosecure_string_generator(x)
            self.assertEqual(len(test_result), x)
